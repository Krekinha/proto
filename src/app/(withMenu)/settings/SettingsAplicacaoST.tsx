...
export default function SettingsAplicacaoST() {
  const { data, error, isLoading } = useSWR(
    `${baseUrl()}/api/nbm-st/listAplicacao`,
    fetcher
  );
  const aplicacao: aplicacaoST[] = data?.capitulos;
  ...
  const parseCSV = async (fileCSV: any) => {
    const lines: aplicacaoST[] = parse(fileCSV as string, {
      columns: ["numero", "descricao"],
      delimiter: "|",
      trim: true,
    ...
    await addAplicacao(lines);
  };

  async function addAplicacao(values: aplicacaoST[]) {
    await nbmSTService.addAplicacoes(values);
  }

...
import { det, NF } from "@/utils/types";
import { useEffect, useState } from "react";

export default function DetailSTNoParam() {
  const { openTab, setOpenTab, selectedNfe, prdSTConfirm } =
    useNFCalcSaidaContext();

  const [prdNoParam, setPrdNoParam] = useState<det[]>([]);

  useEffect(() => {
    let prds: det[] = [];
    prdSTConfirm.forEach((prod) => {
    ...
    const unique = [
      ...new Map(prds.map((item) => [item.prod?.cProd, item])).values(),
    ];
    setPrdNoParam(unique);
  }, [prdSTConfirm]);
